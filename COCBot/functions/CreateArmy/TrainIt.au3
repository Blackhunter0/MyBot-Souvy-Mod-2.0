...
Func TrainIt($iIndex, $howMuch = 1, $iSleep = 400)
	If $g_bDebugSetlogTrain = 1 Then SetLog("Func TrainIt $iIndex=" & $iIndex & " $howMuch=" & $howMuch & " $iSleep=" & $iSleep, $COLOR_DEBUG)
...
Func GetTrainPos(Const $iIndex)
	If $g_bDebugSetlogTrain = 1 Then SetLog("Func GetTrainPos $iIndex=" & $iIndex, $COLOR_DEBUG)
...
Func GetFullName(Const $iIndex, Const $pos)
	If $g_bDebugSetlogTrain = 1 Then SetLog("Func GetFullName $iIndex=" & $iIndex, $COLOR_DEBUG)

	If $iIndex >= $eBarb And $iIndex <= $eBowl Then
		Local $text = ($iIndex >= $eMini ? "Dark" : "Normal")
		If $g_bDebugSetlogTrain = 1 Then Setlog("Troop Name: " & $g_asTroopNames[$iIndex])
...
Func GetRNDName(Const $iIndex, Const $pos)
	If $g_bDebugSetlogTrain = 1 Then SetLog("Func GetRNDName $iIndex=" & $iIndex, $COLOR_DEBUG)
	Local $aReturn[4]
...
Func GetVariable(Const $ImageToUse, Const $iIndex)
	...
	If IsArray($res) Then
		If $g_bDebugSetlog = 1 Then SetLog("DLL Call succeeded " & $res[0], $COLOR_ERROR)
		If $res[0] = "0" Then
