...
Func CheckDisplay()
	...
	Else
		If $g_bDebugSetlog = 1 Then SetLogCentered("  Display DPI setting = " & $iDPIRatio & "  ", "+", $COLOR_INFO)
		...
		For $i = 1 To $aMonitorData[0][0] ; search system display data for display with Android Emulator window on it and check size
			...
			If $aMonitorData[$i][0] = $hMonitor Then ; find display with Android Emulator
				...
				Else
					...
					If $g_bDebugSetlog = 1 Then SetLogCentered(" Display size= " & $sBSDisplaySize & " ", "+", $COLOR_INFO)
					ExitLoop
				EndIf
			EndIf
		Next
