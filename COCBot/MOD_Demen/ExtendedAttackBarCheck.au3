...
Func ExtendedAttackBarCheck($aTroop1stPage, $Remaining)
	...
	If IsArray($res) Then
		...
		Else
			...
			If $g_bDebugSetlog = 1 Then Setlog("$res[0] = " & $res[0])
			...
			For $i = 0 To UBound($aKeys) - 1
				...
				Else
					...
					$aCoordArray[0][1] = -1
				EndIf
				If $g_bDebugSetlog = 1 Then Setlog($aResult[$i][0] & " | $aCoordArray: " & $aCoordArray[0][0] & "-" & $aCoordArray[0][1])
				;;;;;;;; If exist Castle Spell ;;;;;;; - Fix case 2 CC Spells (Demen)
				If UBound($aCoords) > 1 And StringInStr($aResult[$i][0], "Spell") <> 0 Then
					If $g_bDebugSetlog = 1 Then Setlog($aResult[$i][0] & " x" & UBound($aCoords) & " multiple detected: " & $aValue)
			...
			For $i = 0 To UBound($aResult) - 1
				...
				If $aResult[$i][1] > 0 Then
					If $g_bDebugSetlog = 1 Then SetLog("SLOT : " & $i, $COLOR_DEBUG) ;Debug
					If $g_bDebugSetlog = 1 Then SetLog("Detection : " & $aResult[$i][0] & "|x" & $aResult[$i][1] & "|y" & $aResult[$i][2], $COLOR_DEBUG) ;Debug
					...
					If $iDublicateSlot <> -1 Then
						If $g_bDebugSetlog = 1 Then Setlog($aResult[$i][0] & " is already found in 1st page at Slot: " & $aTroop1stPage[$iDublicateSlot][1])
						ContinueLoop
					EndIf
					...
					If UBound($Slottemp) = 2 Then
						If $g_bDebugSetlog = 1 Then SetLog("OCR : " & $Slottemp[0] & "|SLOT: " & $Slottemp[1], $COLOR_DEBUG) ;Debug
			...
			If Not $Remaining Then
				...
				If $g_bDebugSetlog = 1 Then SetLog("$iTotalSlot1stPage : " & $iTotalSlot1stPage, $COLOR_DEBUG) ;Debug
				$g_iTotalAttackSlot = $iSlotExtended + $iTotalSlot1stPage + $iCCSpell
			EndIf

			If $g_bDebugSetlog = 1 Then Setlog("$iSlotExtended / $iCCSpell / $g_iTotalAttackSlot: " & $iSlotExtended & "/" & $iCCSpell & "/" & $g_iTotalAttackSlot)

		EndIf
	EndIf

	If $g_bDebugSetlog = 1 Then Setlog("Extended String: " & $strinToReturn)
